...
describe("useRecording session-ID filtering (characterization / regression)", () => {
  ...
  it("keeps generic timing on a single final result", async () => {
    const { result } = renderHook(() => useRecording());
    await flushMicrotasks();
    ...
    await waitFor(() => {
      ...
      expect(result.current.timing?.asrMs).toBe(42);
      expect(result.current.timing?.polishMs).toBe(900);
      expect(result.current.timing?.totalMs).toBe(948);
