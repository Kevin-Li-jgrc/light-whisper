...
**识别完成但输入框没有文字**：先确认输入框已获得焦点。在“设置 → 快捷键 → 补输入上一段”配置一个全局快捷键；之后点好目标输入框，按下并松开该快捷键，即可用当前输入方式再次插入最近一段最终文字，无需重新识别或润色。缓存只在本次运行内保留，不依赖历史记录；录音、处理或输入期间暂不接受补输入。再次主动触发会再次插入，请先确认是否需要。
...
在“设置 → 智能词库 → 管理词库”中搜索全部词条，每页显示 50 条，支持单条删除和勾选本页批量删除。设置页只预览前 8 条，避免大量词条占用页面。

点击“批量添加”，按每行一个词粘贴文本或 Excel 单列，先预览，再确认添加。词组内部的空格会保留；空行跳过，重复词按忽略大小写、合并多余空白的规则去重。已有词条的权重和来源保持不变，每条最多 80 个字符，多列内容会标为无效。

词库保存不再有 300 条数量上限，批量添加和重新加载不会按条数截断。删除的词不会被自动学习加回，手动重新添加可解除该词的屏蔽。保存容量与模型使用数量分开：每次识别和润色仍按各自已有规则选取热词，保存更多词条不意味着每次都会使用全部词条。
